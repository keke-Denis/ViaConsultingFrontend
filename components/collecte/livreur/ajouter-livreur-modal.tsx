...
import { ajouterLivreur } from "@/lib/livreur/livreur-api"
import type { LivreurFormData, LivreurFromAPI } from "@/lib/livreur/livreur-types"

const COLOR = "#72bc21"

interface AjouterLivreurModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: (livreur: LivreurFromAPI) => void
}

export default function AjouterLivreurModal({ isOpen, onClose, onSuccess }: AjouterLivreurModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const nouveauLivreur = await ajouterLivreur(formData)
      onSuccess(nouveauLivreur)
      handleClose()
    } catch (error) {
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Nouveau Livreur</h2>
                <p className="text-gray-600">Ajoutez un nouveau livreur à votre équipe</p>
              </div>
            </div>
            <button
              onClick={handleClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex-1 px-6 py-3 text-white font-semibold rounded-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50"
